...
class ProfileView(discord.ui.LayoutView):
    ...
    Container (accent = embed_color)
      ...
      Separator
      TextDisplay  — roster header (Pokémon name / nickname / slot / level)
      MediaGallery — roster Pokémon sprite
      Separator
      ActionRow    — ⏮ ◀  [slot N / total]  ▶ ⏭
    """
    ...
    def __init__(
        ...
        self.avatar = avatar
        self.roster = roster           # list of roster dicts from the API
        self.box_rating = box_rating   # dict with keys: total_rating_formatted, paste_url, error
    ...
    def _sprite_url(self) -> str:
        ...
        mon = self._current_mon()
        safe = _safe_name(mon.get("name", "Missingno"))
    ...
    def _profile_url(self) -> str:
        safe_uname = self.username.replace(" ", "%20")
        return f"{HOST}/prof.php?user={safe_uname}"
    ...
    # ------------------------------------------------------------------
    # Build / Rebuild the component tree
    # ------------------------------------------------------------------
